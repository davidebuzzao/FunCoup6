...
[FunCoup 6](https://funcoup.org) is a functional association network tool developed using Python (v3.10.8) and the Django (v4.1.1) framework. It processes biological data to train networks and visualize results in a web application. The data is stored in a PostgreSQL database, and the frontend leverages Bootstrap and D3.js to provide an interactive user experience. FunCoup 6 is containerized and deployed using Docker and Docker Compose for seamless development and production environments.
...
1. **Database Password**: The database password can be anything you choose. For example, you can set it to `"ABCDEF12345"`.
  
2. **Django Secret Key**: To generate a secret key for the Django application, you can use the following command:
...
3. **File Renaming**: After updating the tokens:

   - Rename `FunCoup/settings_repo.py` to `FunCoup/settings.py`.
   - Rename `docker-compose_repo.yml` to `docker-compose.yml`.
...
FunCoup 5 can be reached at [https://funcoup5.scilifelab.se](https://funcoup5.scilifelab.se).
